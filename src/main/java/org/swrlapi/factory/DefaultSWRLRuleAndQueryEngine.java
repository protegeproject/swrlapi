...
class DefaultSWRLRuleAndQueryEngine implements SWRLRuleEngine, SQWRLQueryEngine
{
  ...
  @Override public void exportInferredOWLAxioms() throws SWRLRuleEngineException
  {
    try {
      ...
      writeOWLAxioms2OWLOntology(this.swrlapiOWLOntology.getOWLAxioms(),
        this.builtInBridgeController.getInjectedOWLAxioms());
      // Write OWL axioms inferred by rules.
      writeOWLAxioms2OWLOntology(this.swrlapiOWLOntology.getOWLAxioms(),
        this.ruleEngineBridgeController.getInferredOWLAxioms());
    } finally {
  ...
  private void writeOWLAxioms2OWLOntology(@NonNull Set<@NonNull OWLAxiom> assertedAxioms,
    @NonNull Set<@NonNull OWLAxiom> axioms) throws SWRLRuleEngineException
  {
    OWLInferredAxiomFilter inferredAxiomFilter = new OWLInferredAxiomFilter(assertedAxioms);
    Set<OWLAxiom> filteredAxioms = axioms.stream().filter(a -> !a.accept(inferredAxiomFilter)).collect(Collectors.toSet());
