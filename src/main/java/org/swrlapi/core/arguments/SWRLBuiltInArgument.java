 ...
 * interface representing arguments to swrl built-ins. In addition to the {@link SWRLAPIBuiltInAtom} class, this
 * interface is the SWRLAPI's primary OWLAPI extension point. The {@link SWRLBuiltInArgument} interface represents SWRL
 * built-in atom arguments in the SWRLAPI, which has a wider range of built-in argument types than the OWLAPI. The
 * OWLAPI envisions built-in arguments as simple literals or variables only. In addition to literals and variables, the
 * SWRLAPI allows OWL named objects (classes, individuals, properties, and datatypes) as well as SQWRL collection
 * arguments.
 * <p>
