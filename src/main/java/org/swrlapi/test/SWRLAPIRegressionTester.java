...
public class SWRLAPIRegressionTester
{
  ...
  public void run()
  {
    ...
    SQWRLQueryRenderer renderer = this.sqwrlQueryEngine.createSQWRLQueryRenderer();
    ...
    try {
      ...
      for (SQWRLQuery query : this.sqwrlQueryEngine.getSQWRLQueries()) {
        String queryName = query.getQueryName();
        System.out.print("\n*****Running test " + queryName + "...\n");
        System.out.print(renderer.renderSQWRLQuery(query) + "\n");
        numberOfTests++;
        try {
          SQWRLResult result = this.sqwrlQueryEngine.runSQWRLQuery(queryName);
          if (result.isEmpty()) {
  ...
  private boolean compare(@NonNull SQWRLResult result, @NonNull String expectedResultString) throws SQWRLException
  {
